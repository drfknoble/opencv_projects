...
const bool ol::object_localiser::ORB(const cv::Mat & in1, const cv::Mat & in2, cv::Mat * out1, cv::Mat * out2, ol::matcher matcher)
{
	...
	case ol::BF:
		...
		for (int i = 0; i < descriptors_object.rows; i++)
		{
			if (matches[i].distance <= 0.66 * max_dist)
			{
...
const bool ol::object_localiser::BRISK(const cv::Mat & in1, const cv::Mat & in2, cv::Mat * out1, cv::Mat * out2, ol::matcher matcher)
{
